...
void printMatrix(Matrix x) {
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			printf("%llx - %lf    ", x.q[i * 4 + j], x.x[i * 4 + j]);
		}
		puts("");
	}
}
...
double round_(double h, double range)
{
...
QWORD Matrix_to_QWORD(Matrix m) {
	...
	for (int i = 0; i < 16; i++) {
		...
		xx |= (((QWORD)round_(m.x[i], 10)&c) << (i * 4));//(m.q[i]&c)<<(i*4);//
//            printf("= %llx \n",xx);
	}
	return xx;
}
...
void Cipher::blowfish_encrypt(const BYTE in[], BYTE out[], const BLOWFISH_KEY *keystruct)
	{
		WORD l, r, t; //,i;
	...
	void Cipher::blowfish_decrypt(const BYTE in[], BYTE out[], const BLOWFISH_KEY *keystruct)
	{
		WORD l, r, t; //,i;
	...
	void Cipher::blowfish_key_setup(const BYTE user_key[], BLOWFISH_KEY *keystruct, size_t len)
	{
		BYTE block[8];
		int idx, idx2;
	...
	bool Cipher::Matrix_inv(double m[16], double invOut[16])
	{
		double inv[16], det;
		int i;
	...
	Matrix Cipher::Matrix_mul(Matrix x, Matrix y) {
		int i, j, l, k;
		Matrix res;
		...
		for (i = 0; i < 4; i++) {
			for (j = 0; j < 4; j++) {
				for (l = 0; l < 4; l++) {
					res.x[i * 4 + j] += x.x[i * 4 + l] * y.x[l * 4 + j];
				}
			}
		}
	...
	Part Cipher::blowfish_enc(Part x) {
		Part res;
		for (int i = 0; i < 8; i++) {
			blowfish_encrypt(x.b[i], res.b[i], &bkey);
		}
		return res;
	}
	Part Cipher::blowfish_dec(Part x) {
		Part res;
		for (int i = 0; i < 8; i++) {
			blowfish_decrypt(x.b[i], res.b[i], &bkey);
		}
		return res;
	}
	...
	bool Cipher::setkey(QWORD _key) {
		km = QWORD_to_Matrix(_key);
		if (Matrix_inv(km.x, km_inv.x) == false) {
			return false;
		}
		key = _key;
		BYTE kb[8];
		memcpy(kb, &key, sizeof(kb));
		blowfish_key_setup(kb, &bkey, BLOWFISH_BLOCK_SIZE);

		return true;
	}
	Matrix Cipher::decrypt(Matrix c) {
		Matrix tmp, p;
		Part l, r, k1, k2, temp;
		tmp = c;
		memcpy(&l, &tmp, sizeof(Part));
		memcpy(&r, &tmp.x[8], sizeof(Part));

		memcpy(&k1, &km, sizeof(Part));
		memcpy(&k2, &km.x[8], sizeof(Part));
		...
		memcpy(&tmp, &r, sizeof(Part));
		memcpy(&tmp.x[8], &l, sizeof(Part));
		tmp = Matrix_mul(tmp, km_inv);


		return tmp;
	}
	Matrix Cipher::encrypt(Matrix p) {
		Matrix p_q, p_r, x, c_q, c_r, c;
		Matrix tmp, tmp2;
		Part l, r, temp, k1, k2;
